...
NONCONVERGE_TOKEN = "Reached maximum number of steps without convergence."


class _GeoCalibNonconvergeHandler(logging.Handler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        if NONCONVERGE_TOKEN in msg and record.name.startswith("vipe.priors.geocalib"):
...
def _interpolate_poses(poses_sub: np.ndarray, n_original: int, frame_step: int) -> np.ndarray:
    """Slerp rotation + lerp translation to recover full-length pose sequence."""
    from scipy.spatial.transform import Rotation, Slerp

    n_sub = poses_sub.shape[0]
    sub_times = np.arange(n_sub) * frame_step
    orig_times = np.arange(n_original, dtype=np.float64)

    rotations = Rotation.from_matrix(poses_sub[:, :3, :3])
    slerp = Slerp(sub_times.astype(np.float64), rotations)
    translations = poses_sub[:, :3, 3]

    orig_times_clamped = np.clip(orig_times, sub_times[0], sub_times[-1])
    rot_interp = slerp(orig_times_clamped)

    t_interp = np.zeros((n_original, 3), dtype=poses_sub.dtype)
    for ax in range(3):
        t_interp[:, ax] = np.interp(orig_times_clamped, sub_times, translations[:, ax])

    result = np.zeros((n_original, 4, 4), dtype=poses_sub.dtype)
...
def _save_pose_npy(pose_path: Path, trajectory, n_original: int = 0, frame_step: int = 1) -> None:
    pose_data = trajectory.matrix().cpu().numpy()
    if frame_step > 1 and n_original > pose_data.shape[0]:
        pose_data = _interpolate_poses(pose_data, n_original, frame_step)
...
def _prefetch_worker(
    ...
    for video_path in task_iter:
        if not video_path.exists():
            ...
            out_queue.put((video_path, None, True, 0))
            continue
        if skip_fn(video_path):
            out_queue.put((video_path, None, True, 0))
            continue
        try:
            stream, n_original = make_stream_fn(video_path)
            out_queue.put((video_path, stream, False, n_original))
        except Exception:
            out_queue.put((video_path, None, True, 0))
    out_queue.put(None)
...
def _process_videos(
    ...
    nonconverged_videos: set[str] = set()
    nonconverge_flag = {"hit": False, "message": ""}
    nonconverge_log_path = _nonconverge_log_path(args.output, worker_id)

    def _mark_nonconverge(msg: str) -> None:
        nonconverge_flag["hit"] = True
        nonconverge_flag["message"] = msg

    nonconverge_handler = _GeoCalibNonconvergeHandler(_mark_nonconverge)
    logging.getLogger().addHandler(nonconverge_handler)
    ...
    from vipe.streams.base import StreamProcessor as _StreamProcessor, VideoFrame as _VideoFrame

    class _DownscaleProcessor(_StreamProcessor):
        n_passes_required: int = 1
        def __init__(self, target_size: tuple[int, int]):
            self._target = target_size
        def update_frame_size(self, prev):
            return self._target
        def update_attributes(self, prev):
            return prev
        def __call__(self, frame_idx: int, frame: _VideoFrame) -> _VideoFrame:
            return frame.resize(self._target)

    def _make_stream(video_path: Path):
        """Returns (cached_stream, n_original_frames)."""
        import cv2 as _cv2

        stream_name = _artifact_name(video_path, args.name_from_path)

        probe = _cv2.VideoCapture(str(video_path))
        n_original = int(probe.get(_cv2.CAP_PROP_FRAME_COUNT))
        probe.release()

        seek_range = range(0, -1, frame_step) if frame_step > 1 else None
        raw_stream = RawMp4Stream(video_path, name=stream_name, seek_range=seek_range)

        processors = []
        if max_short_side > 0:
            h, w = raw_stream.frame_size()
            short_side = min(h, w)
            if short_side > max_short_side:
                scale = max_short_side / short_side
                new_h, new_w = int(h * scale), int(w * scale)
                processors.append(_DownscaleProcessor((new_h, new_w)))

        stream = ProcessedVideoStream(raw_stream, processors).cache(
            desc="Reading video stream"
        )
        return stream, n_original
    ...
    prefetch_queue: queue_mod.Queue = queue_mod.Queue(maxsize=1)
    ...
    while True:
        ...
        if item is None:
            break
        video_path, video_stream, skipped, n_original = item
        idx += 1
        local_done += 1
        ...
        try:
            nonconverge_flag["hit"] = False
            nonconverge_flag["message"] = ""
            ...
            if args.save_artifacts:
                ...
                if output.payload is None:
                    ...
                    pose_path, intrinsics_path = _pose_intrinsics_paths(args.output, video_path)
                    do_interp = args.interp and frame_step > 1
                    _save_pose_npy(
                        pose_path, output.payload.trajectory,
                        n_original=n_original if do_interp else 0,
                        frame_step=frame_step if do_interp else 1,
                    )
                    n_out = n_original if do_interp else output.payload.trajectory.matrix().shape[0]
            ...
            if nonconverge_flag["hit"]:
                key = str(video_path)
                if key not in nonconverged_videos:
                    nonconverged_videos.add(key)
                    _write_nonconverge(
                        nonconverge_log_path,
                        video_path,
                        worker_id,
                        frame_step,
                        max_short_side,
    ...
    logging.getLogger().removeHandler(nonconverge_handler)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Take every N-th frame (1 = all frames, 2 = every other frame, 3 = every 3rd frame).",
    )
    parser.add_argument(
        "--no-interp",
        dest="interp",
        action="store_false",
        help="When using --frame-step, do NOT interpolate poses back to original frame count. "
        "Output will have fewer frames (e.g. 61 or 41 instead of 121).",
    )
    parser.add_argument(
        "--max-short-side",
        type=int,
        default=0,
        help="Downscale video so the short side <= this value before processing (0 = no downscale). "
        "E.g. --max-short-side 540 downscales 1920x1080 to 960x540.",
    )
    parser.add_argument(
        "--cudnn-benchmark",
        action="store_true",
...
def _nonconverge_log_path(output_dir: Path, worker_id: int) -> Path:
    warn_dir = output_dir / "vipe_warnings"
    warn_dir.mkdir(parents=True, exist_ok=True)
    return warn_dir / f"geocalib_nonconverged_worker{worker_id}.jsonl"


def _write_nonconverge(
    log_path: Path,
    video_path: Path,
    worker_id: int,
    frame_step: int,
